...
/** Validates original-line semantics without splitting text or constructing output. */
export function validateLineOperations(
  lineCount: number,
  ...
  const insertions = new Map<string, { line: number }>();
  let hasHead = false;
  let hasTail = false;
  ...
  for (const [index, operation] of operations.entries()) {
    ...
    if (operation.op === "head" || operation.op === "tail") {
      if (operation.op === "head") {
        if (hasHead) throw new Error("Only one head operation is allowed");
        hasHead = true;
      } else {
        if (hasTail) throw new Error("Only one tail operation is allowed");
        hasTail = true;
      }
      continue;
    }
    ...
    if (operation.op === "insert-before" || operation.op === "insert-after") {
      assertLine(operation.line, lineCount, label);
      ...
      insertions.set(key, { line: operation.line });
      continue;
    }
    ...
    assertLine(operation.start, lineCount, label);
    assertLine(operation.end, lineCount, label);
    ...
    let overlappingLine: number | undefined;
    for (const range of ranges) {
      const overlapStart = Math.max(operation.start, range.start);
      if (overlapStart <= Math.min(operation.end, range.end)) {
        overlappingLine = Math.min(
          overlappingLine ?? overlapStart,
          overlapStart,
        );
      }
    }
    if (overlappingLine !== undefined) {
      throw new Error(`${label} overlaps original line ${overlappingLine}`);
    }
    ranges.push({ start: operation.start, end: operation.end });
  }

  for (const insertion of insertions.values()) {
    if (
      ranges.some(
        (range) => insertion.line >= range.start && insertion.line <= range.end,
      )
    ) {
      throw new Error(
...
export function applyLineOperations(
  ...
  validateLineOperations(original.lines.length, operations, seenLines);
  ...
  for (const operation of operations) {
    switch (operation.op) {
      ...
      case "insert-after": {
        const side = operation.op === "insert-before" ? "before" : "after";
        insertions.set(`${side}:${operation.line}`, {
          line: operation.line,
          side,
